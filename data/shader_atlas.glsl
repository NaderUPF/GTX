...
// G-BUFFER FILL SHADER (HDR PBR version)
pbr_gbuffer_fill basic.vs pbr_gbuffer_fill.fs

// DEFERRED LIGHTING SHADER (HDR PBR version)
...
void main()
{
    ...
    // Roughness, metallic and AO could be packed in other targets or channels if needed
}
...
void main()
{
    ...
    vec3 albedo = pow(albedo_sample_srgb.rgb, vec3(2.2)); // sRGB to linear
    float alpha = albedo_sample_srgb.a;

    vec4 normal_sample = texture(u_normal_material_texture, uv);
    vec4 mr_sample = texture(u_depth_texture, uv); // if metallic_roughness in another texture, replace accordingly
